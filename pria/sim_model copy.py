...
class Trainer:
    ...
    def read_meta_data(self):
        ...
        self.train_index = self.total_images * 70 // 100
        self.val_index = self.total_images * 20 // 100 + self.train_index
...
>>>>>>> origin/main
    ...
    def normalize_data(self):
        ...
        self.max_imgs = np.max(self.shuffled_imgs[:, :, :, :], axis=0)
        self.min_imgs = np.min(self.shuffled_imgs[:, :, :, :], axis=0)

        self.max_labels = np.max(self.shuffled_labels[:, :], axis=0)
        self.min_labels = np.min(self.shuffled_labels[:, :], axis=0)
        # print(self.max_imgs.shape)
    ...
    def metrics(self):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(self.Yv), self.batch_size):
                ...
                np.append(
                    values, np.array((vy_u - test_y_hat_u.detach().numpy())), axis=0
                )
...
=======
            ...
            self.train_error[j] /= int(len(self.Yt) / self.batch_size)
...
=======
                ...
                self.val_error[j] /= int(len(self.Yv) / self.batch_size)
