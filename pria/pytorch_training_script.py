...
from pria.pytorch_model_image_to_coarse import *
...
class Trainer:
    def __init__(self, data_folder, epochs):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        options = ['flat_no_twist', 'flat_twist', 'cone_no_twist', 'cone_twist']
        data_folder = os.path.join("dataset", sys.argv[1], options[0])

        if len(sys.argv) == 4:
            Trainer(data_folder, epochs).infer(int(sys.argv[4]))
        else:
            # Trainer(data_folder, epochs).run_batch()
            for i in range(4):
                data_folder = os.path.join("dataset", sys.argv[1], options[i])
        ...
        # Trainer(data_folder, epochs).gt_converter()
